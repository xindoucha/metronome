...
function renderDrumScores(){
  document.querySelectorAll('[data-score]').forEach(box=>{
    ...
    if(!pattern)return;
    box.innerHTML=drumScoreSvg(pattern);
  });
}

function drumScoreSvg(pattern){
  const xs=Array.from({length:16},(_,i)=>86+i*18);
  const staffTop=30, gap=9, staffBottom=staffTop+gap*4;
  const y={hh:18,sd:staffTop+gap*2,bd:staffTop+gap*4+12};
  const lines=Array.from({length:5},(_,i)=>`<line class="staff-line" x1="64" y1="${staffTop+i*gap}" x2="372" y2="${staffTop+i*gap}"/>`).join('');
  const barlines=[64,136,208,280,372].map(x=>`<line class="staff-barline" x1="${x}" y1="${staffTop}" x2="${x}" y2="${staffBottom}"/>`).join('');
  const counts=[0,4,8,12].map((idx,i)=>`<text class="staff-count" x="${xs[idx]}" y="82" text-anchor="middle">${i+1}</text>`).join('');
  const notes=[
    ...pattern.hh.map(i=>xNote(xs[i],y.hh)),
    ...pattern.sd.map(i=>roundNote(xs[i],y.sd,true)),
    ...pattern.bd.map(i=>roundNote(xs[i],y.bd,true,true))
  ].join('');
  const stems=[
    ...pattern.hh.map(i=>`<line class="staff-stem" x1="${xs[i]+5}" y1="${y.hh+1}" x2="${xs[i]+5}" y2="48"/>`),
    ...pattern.sd.map(i=>`<line class="staff-stem" x1="${xs[i]+5}" y1="${y.sd}" x2="${xs[i]+5}" y2="48"/>`),
    ...pattern.bd.map(i=>`<line class="staff-stem" x1="${xs[i]-5}" y1="${y.bd}" x2="${xs[i]-5}" y2="45"/>`)
  ].join('');
  const beams=(pattern.beams||[]).map(([start,end,count])=>{
    const x1=xs[start]+5, x2=xs[end]+5;
    const first=`<line class="staff-beam" x1="${x1}" y1="47" x2="${x2}" y2="47"/>`;
    const second=count>1?`<line class="staff-beam" x1="${x1}" y1="53" x2="${x2}" y2="53"/>`:'';
    return first+second;
  }).join('');
  return `
    <svg class="drum-staff" viewBox="0 0 390 92" role="img" aria-hidden="true">
      <text class="staff-label" x="14" y="17">HH</text>
      <text class="staff-label" x="14" y="51">SD</text>
      <text class="staff-label" x="14" y="77">BD</text>
      <rect class="staff-clef" x="39" y="33" width="4" height="26" rx="1" fill="none" stroke-width="2"/>
      <rect class="staff-clef" x="47" y="33" width="4" height="26" rx="1" fill="none" stroke-width="2"/>
      <text class="staff-time" x="57" y="43" text-anchor="middle">4</text>
      <text class="staff-time" x="57" y="61" text-anchor="middle">4</text>
      ${lines}${barlines}${stems}${beams}${notes}${counts}
    </svg>`;
}

function xNote(cx,cy){
  return `<g class="staff-x"><line x1="${cx-5}" y1="${cy-5}" x2="${cx+5}" y2="${cy+5}"/><line x1="${cx+5}" y1="${cy-5}" x2="${cx-5}" y2="${cy+5}"/></g>`;
}
function roundNote(cx,cy,filled=false,down=false){
  const cls=filled?'staff-note filled':'staff-note';
  const rotate=down?20:-20;
  return `<ellipse class="${cls}" cx="${cx}" cy="${cy}" rx="5.5" ry="4" transform="rotate(${rotate} ${cx} ${cy})"/>`;
}
